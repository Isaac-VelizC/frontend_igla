...
import { get_docentes_list } from "./../../../redux/actions/users/users";
...
import { deleteItem, guardarInformacion, actualizarInformacion } from './../../services/usuarios';
import FormDialog from 'components/dialog/dialogForm';

function Docentes({ get_docentes_list, docentes }) {
    let emptyDocentes = {
        id: null,
        nombre: '',
        ap_paterno: '',
    ...
    const [docente, setDocente] = useState(emptyDocentes);
    ...
    const confirmDeleteDocente = (docente) => {
        setDocente(docente);
    ...
    const actionBodyTemplate = (rowData) => {
        return (
            <>
                <button title='Editar' type="button" onClick={() => editDocente(rowData)}
                ...
                <button title='Borrar' type="button" onClick={() => confirmDeleteDocente(rowData)}
    ...
    const deleteDocenteDialogFooter = (
        <>
        <button type="button" onClick={hideDeleteDocenteDialog}
            className="text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-500 dark:focus:ring-blue-800">
                No
        </button>
        <button type="button" onClick={(e) => onSubmitDelete(e, docente)}
    ...
    const saveDocente = async (e) => {
        try {
            setSubmitted(true);
            if (isValidEmail(docente.email)) {
                await checkEmailUniqueness(docente.email, docente.id);
            } else {
                setIsEmailUnique(false);
            }
            if (isValidCi(docente.ci)) {
                await checkCiUniqueness(docente.ci, docente.id);
            } else {
                setIsCiUnique(false);
            }
            await checkTelefonoUniqueness(docente.telefono, docente.id);
            if (isEmailUnique && isCiUnique && isTelefonoUnique) {
                if (docente.id && docente.id > 0) {
                    await onSubmitActualizar(e, docente);
                } else if (docente.id === null) {
                    await onSubmitGuardar(e, docente)
                } else {
    ...
    const onSubmitGuardar = (e, slug) => {
        e.preventDefault();
        guardarInformacion(slug, get_docentes_list, successDeleteDocente, errorDeleteDocente);
    };

    const onSubmitActualizar = (e, slug) => {
        e.preventDefault();
        actualizarInformacion(slug, get_docentes_list, successDeleteDocente, errorDeleteDocente);
    };

    const docenteDialogFooter = (
        <>
            <Button label="Cancela" className='h-10' outlined onClick={hideDialog} />
            <Button label="Guardar" className='h-10' onClick={saveDocente}/>
        </>
    );

    const onSubmitDelete = (e, slug) => {
        e.preventDefault();
        deleteItem(slug, get_docentes_list, successDeleteDocente, errorDeleteDocente);
    };
    
    const successDeleteDocente = (message) => {
        hideDialog();
        setDeleteDocenteDialog(false);
        setDocente(emptyDocentes);
    ...
    const errorDeleteDocente = (message) => {
        setDeleteDocenteDialog(false);
        setDocente(emptyDocentes);
    ...
    return (
        <div>
            ...
            <div className="relative px-5 overflow-x-auto shadow-md sm:rounded-lg">
                ...
                <DataTable ref={dt} value={docentes} paginator rows={10}
            ...
            <FormDialog
                ...
                formDialogFooter={docenteDialogFooter}
            />

            <Dialog visible={deleteDocenteDialog} style={{ width: '32rem' }} breakpoints={{ '960px': '75vw', '641px': '90vw' }} header="Confimación" modal footer={deleteDocenteDialogFooter} onHide={hideDeleteDocenteDialog}>
                <div className="confirmation-content">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {docente && (
                        <span>
                            ¿Estás seguro de que quiere dar de {docente.estado == 1 ? 'baja' : 'alta'} a <b>{docente.nombre + ' ' + docente.ap_paterno}</b>?
                        </span>
                    )}
                </div>
            </Dialog>
